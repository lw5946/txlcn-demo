...
//    @TccTransaction(confirmMethod = "confirmRpc",cancelMethod = "cancelRpc", propagation = DTXPropagation.SUPPORTS)
    ...
    public String insert(String value, String exFlag) {
        log.info("执行c.insert({},{})", value, exFlag);
        Demo demo = new Demo();
        demo.setDemoField("c:" + value);
        demoMapper.save(demo);

        if (ExFlagEnum.SERVICE_C_BEFORE.getFlag().equals(exFlag)) {
       ...
       /* ids.putIfAbsent(TracingContext.tracing().groupId(), Sets.newHashSet(demo.getId()));
        ...
        if (ExFlagEnum.SERVICE_C_AFTER.getFlag().equals(exFlag)) {
            throw new IllegalStateException("by exFlag: "+ exFlag);
        }*/
        return "ok-service-c";
    }
...
//    @TccTransaction(confirmMethod = "confirmRpc",cancelMethod = "cancelRpc", propagation = DTXPropagation.SUPPORTS)
    ...
    public String update(long id, String value, String exFlag) {
        log.info("执行c.update({},{})", value, exFlag);
        Demo demo = new Demo();
        demo.setDemoField("c:" + value);
    ...
    public void confirmRpc(String value, String exFlag) {
    ...
    public void cancelRpc(String value, String exFlag) {
        ids.get(TracingContext.tracing().groupId()).forEach(id -> {
